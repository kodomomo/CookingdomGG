...
const Header: FC<Props> = ({
    ...
    const getHeaderTypes =  () => {
        ...
        type.forEach((t, i) => {
            headers.push(
                <div className={t.title == searchType ? "selected" : ""} onClickCapture={() => changeSearchType(t.title)}>
                    {t.title}
                    <ul>
                        {t.sub_list.map((sub) => (
                            <li>
                                <button className={sub == t.value ? "selected" : ""} onClick={() => changeSearchDetail(t.title, sub)}>
                                    {typeToString[sub]}
                                </button>
                            </li>)
                        )}
        ...
        headers.push(
            <div className={"직업" == searchType ? "selected" : ""} onClickCapture={() => changeSearchType("직업")}>
                직업
                <ul>
                    {Majors.map((sub) => (
                        <li>
                            <img className={major.includes(sub as majorType) ? "selected" : ""} onClick={() => changeSearchDetail("직업", sub)}
    ...
    return (
        <div className="cookielist__header">
            ...
            <div className="cookielist__header__type">
                {getHeaderTypes()}
            </div>
            <div className="">
